...
MQTTStimer = tmr.create()
   ...
   m:connect("10.10.10.4", 1883, false, function(client, reason)
      ...
      pubtimer:alarm(5000, tmr.ALARM_AUTO, function()
        if gpio.read(so1) == 1 then
            client:publish("5", "101", 1, 1, function(client) end)
            print("so1 on")
        else
            client:publish("5", "100", 1, 1, function(client) end)
            print("so1 off")
        end
        if gpio.read(so2) == 1 then
            client:publish("5", "201", 1, 1, function(client) end)
            print("so2 on")
        else
            client:publish("5", "200", 1, 1, function(client) end)
            print("so2 off")
        end
        if gpio.read(so3) == 1 then
            client:publish("5", "301", 1, 1, function(client) end)
            print("so3 on")
        else
            client:publish("5", "300", 1, 1, function(client) end)
            print("so3 off")
        end
        if gpio.read(so4) == 1 then
            client:publish("5", "401", 1, 1, function(client) end)
            print("so4 on")
        else
            client:publish("5", "400", 1, 1, function(client) end)
            print("so4 off")
        end
      end)
   end,
